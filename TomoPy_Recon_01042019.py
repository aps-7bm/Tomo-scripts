...
parent_directory = os.getcwd +"/Fister/"
centers_file = parent_directory+ "Cycle2018_3_Centers_Fister.txt"
data_key = "ALL"
...
def fremove_stripes(dataset,method='sf'):
    ...
    if method == 'fw':
        ...
        dataset = tomopy.prep.stripe.remove_stripe_ti(dataset,ncore=ncore) # Titarenko algorithm
    else:
...
def fwrite_tiff_stack(recon_dataset,smin,total_rows):
    #Write 16 or 32-bit tiff stack out
    t0=time.time()
    ...
    print(("Writing slices {0:d}-{1:d} to {2:s}...".format(start_write_row, last_write_row - 1,output_path_tiffstack)))    
...
def calculate_chunksize(rowmin,rowmax):
    if rowmin is None:
        rowmin = 0
    if rowmax is None:
        rowmax = r
    ...
    im_size = (4*p*c)/(1e9) #image gets converted to 32 bit i.e. 4 bytes per bit
    pad_size_im = (im_size*pad_fraction_width)*2+im_size #size after padding occurs
    set_size = (4*p*r*c)/(1e9) #dataset gets converted to 32 bit i.e. 4 bytes per bit
    pad_size = (set_size*pad_fraction_width)*2+set_size #size after padding occurs
    #biggest memory bottleneck is the padded chunk getting copied on all used cores
    chunkfrac = threshold/(pad_size*(ncore/2))
    rchunk = int(chunkfrac*r)
    return rowmin,rowmax,rchunk
    
#find the files on different computers 
def find(name, path):
...
def ftomopy_script():
    ...
    if beam_hardening:
        ...
        #plt.show()
        #bh.sample_material = bh.Si_filter
        global coeffs
...
#    plt.show()
    ...
    ta = time.time()
    print(('\nWorking on slices {0:d} to {1:d} of dataset ({2:d} slices total)'.format(start_row,end_row,projection_data_full.shape[1])))

    #Stripe removal
    projection_data_full = fremove_stripes(projection_data_full)

    #Correct for beam hardening, if desired. Otherwise, convert to pathlength
    if beam_hardening:
        projection_data_full = fapply_beam_hardening_projection(projection_data_full)
    else:
    ...
    if (c-1)/2 > rotation_center:
        center_location = 'left'
    else:
        center_location = 'right'
    working_data_subset, working_theta, working_rotation_center = freregister_data(projection_data_full,theta,rotation_center,center_location,step_theta)
    ...
    t0=time.time()
    print("\nRemoving rings...")
    data_recon = tomopy.remove_ring(data_recon.copy(),center_x = rotation_center,center_y = rotation_center,ncore=ncore)
    ...
    if output_path_tiffstack is not None:
        fwrite_tiff_stack(recon_dataset = data_recon,smin=0,total_rows = r)
...
if __name__ == "__main__":
    ...
    print(("Using TomoPy version = " + tomopy.__version__))
    for cond in dat:
        if cond.startswith("#"):
            pass
        else:
            start_time = time.time()
            file_path,data_filename = find(cond.split(' ')[0],parent_directory)
            rotation_center = float(cond.split(' ')[1][:-2])       
            #create location for saving data
            basename = os.path.basename(os.path.normpath(file_path))
            output_path_tiffstack = file_path+"/Recon_"+basename+"/" # None for no TIFF output
            print("Working on dataset %s" %basename)
            ...
            rowmin, rowmax,rchunk = calculate_chunksize(row_start,row_end)
            print("Chunk size is %i"%rchunk)
            ...
            for rows in np.arange(rowmin,rowmax,rchunk):
                ...
                if end_col is None:
                   end_col = c 
                ftomopy_script()
            print('Total time for recon of %s = %.1f min' % (basename,(time.time() - start_time)/60.))
